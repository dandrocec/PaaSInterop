...
public class GuiModelBuilder {
    ...
    public static OntologyNode buildFromRDF(String data) throws Exception {
        ...
        for(OntResource rootCls : sortContent(collectRootConcepts(ontoModel).iterator())) {
    ...
    private static ModelNode processOntResource(OntResource resource) {
    	try {
    		if (resource instanceof OntClass) {
    			...
    			for(OntResource propIt : sortContent(oClass.listDeclaredProperties(true)))	{
    				ModelNode subModel = processOntResource(propIt);
    				if (subModel != null) {
    					cNode.add(subModel);
    				}
    			} 
    			//    		for(OntClass subConcept : listNamedSubClasses(oClass)) {
    			for(OntResource subConcept : sortContent(listNamedSubClasses(oClass).iterator())) {
    			...
    			for(OntResource it : sortContent(oClass.listInstances(true))) {
    				ModelNode subModel = processOntResource(it);
    				if (subModel != null) {
    					cNode.add(subModel);
    				}
    			}
    ...
    public static List<OntResource> sortContent(Iterator<? extends OntResource> inputs) {
		
    	List<OntResource> result = new ArrayList<OntResource>();
    	
    	Map<String, OntResource> buffer = new HashMap<String, OntResource>();
    	final Map<String, String> dict = new HashMap<String, String>();
    	
    	while (inputs.hasNext()) {
    		OntResource resource = inputs.next();
    		String uri = resource.getURI();
    		if (uri == null) {
    			continue;
    		}
    		String lab = getLabel(uri);
    		buffer.put(uri, resource);
    		dict.put(uri, lab);
		}
    	String[] keys = dict.keySet().toArray(new String[dict.size()]);
    	Arrays.sort(keys, new Comparator<String>() {
			public int compare(String o1, String o2) {
				return dict.get(o1).compareToIgnoreCase(dict.get(o2));
			}
		});
    	for(String uri : keys) {
    		result.add(buffer.get(uri));
    	}
    	return result;
    }
